...
function resolveChannelId(guildId, input) {
    // If input looks like a number, assume it's already an ID
    if (/^\d+$/.test(input)) {
        return input;
    }
...
async function ensureServerRunning(guildId, channelId) {
    if (fs.existsSync(SOCKET_PATH)) {
...
program
    ...
    .action(async (guildId, channelId) => {
    try {
        loadConfig();
        ...
        if (!/^\d+$/.test(channelId)) {
            ...
            const channel = channels.find((ch) => ch.name.toLowerCase() === channelId.toLowerCase() && ch.type === 2);
            if (!channel) {
